...
from transformers.models.big_bird.modeling_big_bird import BigBirdEmbeddings, BigBirdLMPredictionHead, \
    BigBirdOnlyMLMHead,BigBirdPredictionHeadTransform
...
class plTrainHarness(pl.LightningModule):
    ...
    def cal_oacc(self, opreds, answers, ocorrs, ototas, train=True):
        ...
        if train:
            ...
            if self.organism_trclacc==[]:
                ...
                self.organism_trclacc.append((float(correct / total) + len(self.organism_trclacc)*self.organism_trclacc[-1]) / (len(self.organism_trclacc) + 1))
    ...
    def cal_gc_and_cai(self, preds2, types, train=True, focus=None):
        ...
        if train and focus is not None:
            ...
            ref_cais, ref_gcs = [], []
            for i in range(len(pred_seqs)):
                seq = [INDEX2TOKEN[li].split(STOP_SYMBOL)[-1] if li >= 0 else '' for li in focus[i]]
                """
                if seq.count('unk')/len(seq)>0.05:
                    print('UNK percentage is over 5%, invalid label.')
                    continue
                seq = ''.join(seq).replace('unk','')
                """
                seq=''.join(seq)
                if seq == '' or 'unk' in seq:
                    continue
                marks.append(i)
                ref_cais.append(CAI.CAI(seq, weights=self.codon_table[self.id2organism[int(types[i])]], genetic_code=11))
            ...
            for i in range(NUM_ORGANISMS):
                ...
                if len(self.ref_trgcs[i]) > 0:
                    # 2026.3.14分母调整：特殊数据集可能出现序列标签含有unk过多或者短标签全部未选中情况，因此实际分母可能更小
                    self.ref_trgcs[i].append((self.ref_trgcs[i][-1] * len(self.ref_trgcs[i]) + gc_record[i] /(number + 1e-3)) / (len(self.ref_trgcs[i]) + 1))
                    self.ref_trcais[i].append((self.ref_trcais[i][-1] * len(self.ref_trcais[i]) + cai_record[i] /(number + 1e-3)) / (len(self.ref_trcais[i]) + 1))
                else:
                    self.ref_trgcs[i].append(gc_record[i] / (number + 1e-3))
                    self.ref_trcais[i].append(cai_record[i] / (number + 1e-3))
            ...
            for i in range(len(pred_seqs)):
                ...
                cais.append(CAI.CAI(pred_seqs[i], weights=self.codon_table[self.id2organism[int(types[i])]], genetic_code=11))
            ...
            for i in range(NUM_ORGANISMS):
                if len(self.trgcs[i]) > 0:
                    self.trgcs[i].append((self.trgcs[i][-1] * len(self.trgcs[i]) + gc_record[i] / ((types == i).float().sum().detach().cpu().numpy() + 1e-3)) / (len(self.trgcs[i]) + 1))
                    self.trcais[i].append((self.trcais[i][-1] * len(self.trcais[i]) + cai_record[i] / ((types == i).float().sum().detach().cpu().numpy() + 1e-3)) / (len(self.trcais[i]) + 1))
                else:
            ...
            for i in range(len(pred_seqs)):
                ...
                """
                if seq.count('unk')/len(seq)>0.05:
                    print("UNK percentage of the label DNA sequence is over 5%.")
                    continue
                seq = ''.join(seq).replace('unk','')
                """
                if 'unk' in seq:
                ...
                self.ref_vacais[types[i]][-1][1] += (CAI.CAI(seq, weights=self.codon_table[self.id2organism[int(types[i])]], genetic_code=11))
            ...
            for i in range(len(pred_seqs)):
                ...
                self.vacais[types[i]][-1][1] += (CAI.CAI(pred_seqs[i], weights=self.codon_table[self.id2organism[int(types[i])]], genetic_code=11))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        bp_acc, count = 0, 0
        for i in range(len(pred_seqs)):
            seq = [INDEX2TOKEN[int(li)].split(STOP_SYMBOL)[-1] if li > 4 else '' for li in batch2.get('labels')[i]]
            if 'unk' in seq:
                continue
            """
            # 针对比如毕赤酵母微调数据集的特殊情况，如果每个序列都有unk但是只在终止密码子或特殊位置，纳入CAI和GC计算
            # 为保证代码鲁棒性，不仅仅针对最后三位切除，使用一般方法
            if seq.count('unk')/len(seq)>0.05:
                print(f'UNK percentage:{seq.count("unk")/len(seq)}')
                continue
            
            temp_pred=['']
            temp_pred.extend([pred_seqs[i][j:j+3] for j in range(0,len(pred_seqs[i]),3)])
            for j in range(len(seq)-len(temp_pred)):
                temp_pred.append('')
            temp_pred=np.array(temp_pred)
            assert(len(temp_pred)==len(seq))
            temp_pred=np.where(np.array(seq)=='unk','',temp_pred)
            temp_pred=''.join(list(temp_pred))
            seq=''.join(seq).replace('unk','')
            """
            temp_pred=pred_seqs[i]
            seq=''.join(seq)

            ref_cais.append(CAI.CAI(seq, weights=self.codon_table[self.id2organism[int(batch2["token_type_ids"][i, 0])]],genetic_code=11))
            ref_gcs.append(len(re.sub(r'[^gc]+', '', seq)) / len(seq))
            assert (len(temp_pred) == len(seq))
            for pi, si in zip(temp_pred, seq):
                bp_acc += float(pi == si)
            count += len(seq)
...
class CustomBigBirdEmbeddings(BigBirdEmbeddings):
    def __init__(self, config, organism_embed_dim=32, kp=1, asym=False, step=1):
        super().__init__(config)
        self.token_type_embeddings=nn.Embedding(config.type_vocab_size,organism_embed_dim)
        self.asym = asym
        self.kp = kp
        self.count = 0
        self.step = step
...
class LateFusionHead(BigBirdPredictionHeadTransform):
    def __init__(self, config,organism_embed_dim=32):
        super().__init__(config)
        self.dense = nn.Linear(config.hidden_size+organism_embed_dim, config.hidden_size)
...
class MyCodonDecoderHead(BigBirdLMPredictionHead):
    def __init__(self, config,organism_embed_dim=32):
        super().__init__(config)
        self.transform=LateFusionHead(config,organism_embed_dim)
        self.decoder_organism_bias = nn.Parameter(torch.zeros(config.type_vocab_size,requires_grad=False))
    ...
    def set_decoder_organism(self, new_embeddings):
        if isinstance(new_embeddings,nn.Module):
            assert(self.decoder_organism.weight.shape==new_embeddings.weight.T.shape)
            self.decoder_organism.weight = new_embeddings.weight.T
...
class MyCodonBBMLMHead(BigBirdOnlyMLMHead):
    def __init__(self, config, organism_embed_dim=32):
        super().__init__(config)
        self.predictions = MyCodonDecoderHead(config, organism_embed_dim)
...
class MyBigBirdModel(BigBirdForMaskedLM):
    ...
    def __init__(self, config, organism_embed_dim=32,kp=1, scale=1, asymptotic=False, step=1, type_lf=1):
        super().__init__(config)
        self.organism_embed_dim=organism_embed_dim
        ...
        self.bert.embeddings=CustomBigBirdEmbeddings(config,self.organism_embed_dim,self.kp,
                                                                 self.asymptotic, self.step)
        self.cls = MyCodonBBMLMHead(config,organism_embed_dim)
    ...
    def forward(
        ...
        sequence_output=torch.cat([token_type_embeddings,sequence_output],dim=2)
        ...
        if labels is not None and token_type_ids is not None:
            ...
            masked_lm_loss = loss_fct(prediction_scores, labels)+self.type_lf*loss_fct(prediction_organism_scores.view(-1,prediction_organism_scores.shape[-1]),
                                                                          tlabels.view(-1))
...
def main(args):
    ...
    model = MyBigBirdModel(config=config, organism_embed_dim=32, kp=1, scale=1, asymptotic=False, step=1, type_lf=0.01)
    ...
    if args.ckpt_path!="":
        if os.path.isfile(args.ckpt_path):
            harnessed_model=harnessed_model.load_from_checkpoint(args.ckpt_path,model=model,tokenizer=tokenizer,learning_rate=args.learning_rate,warmup_fraction=args.warmup_fraction,
                                                                 config=config,valid_interval=args.valid_check_interval,accumulation_steps=args.accumulate_grad_batches,scaling=15)
        else:
            raise FileExistsError("Checkpoint file not existent.")
    # Load the training data
    train_data = IterableJSONData(args.train_data_path, dist_env="slurm")
    test_data = IterableJSONData(args.test_data_path, dist_env="slurm")
    ...
    dataloader = MyDataLoader(
        ...
        collate_fn=MaskedTokenizerCollator(tokenizer, prob=0.3),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to the validation and testing data JSON file",
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
