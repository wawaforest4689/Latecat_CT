...
from protclust import cluster,split,set_verbosity


ORGANISM2ID2: Dict[str,int]={
...
ID2ORGANISM2:Dict[int,str]={v:k for k,v in ORGANISM2ID2.items()}
    ...
    if isinstance(dataset, str):
        ...
        if os.path.isfile(dataset):
            if dataset.split('.')[-1] == 'tsv':
                dataset = pd.read_csv(dataset, sep='\t',dtype=str)
            elif dataset.split('.')[-1] == 'csv':
                dataset = pd.read_csv(dataset, sep=',',dtype=str)
            # for testing and inference
            elif dataset.split('.')[-1]=='xlsx':
                dataset=pd.read_excel(dataset,sheet_name='Sheet1',header=0)
                mark=True
            dataset=dataset.dropna(subset=['RefSeq_aa']).fillna("")
            ...
            for file in os.listdir(dataset):
                ...
                ds=None
                if file.split('.')[-1] == 'tsv':
                    ds = pd.read_csv(os.path.join(dataset,file), sep='\t',dtype={"RefSeq_nn":str, "RefSeq_aa":str, "Organism":str})
                elif file.split('.')[-1] == 'csv':
                    ds = pd.read_csv(os.path.join(dataset,file), sep=',',dtype={"RefSeq_nn":str, "RefSeq_aa":str, "Organism":str})
                else:
                    continue
                print(type(ds))
                assert(isinstance(ds,pd.DataFrame) or isinstance(ds,pd.io.parsers.readers.TextFileReader))
                temp_ds=pd.concat([temp_ds,ds.dropna(subset=['RefSeq_aa','RefSeq_nn','Organism'])],axis=0)
            dataset=temp_ds


    if not mark:
        required_columns = {"RefSeq_nn", "RefSeq_aa", "Organism"}
        if not required_columns.issubset(dataset.columns):
            raise ValueError(f"Input dataset must have columns: {required_columns}")

        assert (isinstance(dataset.iloc[0, dataset.columns.get_loc('RefSeq_nn')], str))
        # Prepare the dataset for finetuning
        dataset["codons"] = dataset.apply(
            lambda row: get_merged_seq(row["RefSeq_aa"], row["RefSeq_nn"], separator="_"), axis=1
        )

        organisms = set(dataset.loc[:, "Organism"])
        print("organisms detected: ", organisms)
        print("organisms count:", len(organisms))
        # Replace organism str with organism id using ORGANISM2ID
        dataset["organism"] = dataset["Organism"].apply(
            lambda org: process_organism(org, ORGANISM2ID2)
        )

        # Save the dataset to a JSON file
        dataframe_to_json(dataset[["codons", "organism"]], output_file, shuffle=shuffle,infer=False)

    else:
        # dataset["RefSeq_nn"]=["" for i in range(len(dataset))]
        # dataset = dataset.reindex(columns=dataset.columns.tolist() + ["RefSeq_nn"], fill_value="")
        required_columns = {"RefSeq_aa"}
        if not required_columns.issubset(dataset.columns):
            raise ValueError(f"Input dataset must have columns: {required_columns}")

        dataset.rename(columns={"RefSeq_aa":"protein"},inplace=True)

        # Save the dataset to a JSON file
        dataframe_to_json(dataset, output_file, shuffle=shuffle,infer=True)

def dataframe_to_json(df: pd.DataFrame, output_file: str, shuffle: bool = True, infer: bool = False) -> None:
    """
    ...
    if not infer:
        ...
        # Shuffle the DataFrame if requested(here it is not suggested due to requirements)
        if shuffle:
            df = sk_shuffle(df)
        ...
        with open(output_file, "w") as f:
            for idx, row in tqdm(
                ...
                for key in ORGANISM2ID2.keys():
                    print(row["id"])
                    doc = {"idx": row["id"], "protein": row["protein"],"organism":key}
                    f.write(json.dumps(doc) + "\n")
                    count+=1

    print(f"\nTotal Entries Saved: {count}, JSON data saved to {output_file}")
...
def process_organism(organism: Union[str, int], organism_to_id: Dict[str, int]) -> int:
    ...
    if isinstance(organism, str):
        if organism not in organism_to_id:
            if 'Escherichia' in organism:
                organism='Escherichia coli'
            elif 'Pichia' in organism:
                organism='Pichia angusta'
            elif 'Saccharomyces cerevisiae' in organism:
...
def preprocess_protein_sequence(protein: str) -> str:
    ...
    # Default behaviour:(maybe) "standardize_deterministic"
...
def prepare_data_from_fasta(filepath,organism="Pichia angusta",savepath='training_pichia.jsonl',read_len=10000):
    dataset=read_fasta_file(filepath,organism=organism,read_len=read_len)
    print(f"actual reading length:{len(dataset)}")
    dataset.rename(columns={'dna':'RefSeq_nn','protein':'RefSeq_aa','organism':'Organism'},inplace=True)
    prepare_training_data(dataset,savepath,True)
    return IterableJSONData(savepath)

def prepare_data_from_fasta_for_infer(filepath,organism="",savepath='testing_demo.jsonl',read_len=10000):
    dataset=read_fasta_file_for_infer(filepath,organism=organism,read_len=read_len)
    print(f"actual reading length:{len(dataset)}")
    dataframe_to_json(dataset,savepath,False,True)
    return IterableJSONData(savepath)


def read_fasta_file_for_infer(
    input_file: str,
    organism: str = "",
    read_len:int=10000
) -> pd.DataFrame:
    ...
    with open(input_file, "r") as fasta_file:
        for id,record in tqdm(
            enumerate(SeqIO.parse(fasta_file, "fasta")),
            desc=f"Processing {organism}",
            unit=" Records",
        ):
            pro = str(record.seq).strip().upper()  # Ensure uppercase DNA sequence
            print("protein:",pro)

            # Create a data row for the current sequence
            store.setdefault("protein",[]).append(pro)
            store.setdefault("id",[]).append(id+1)
            if id+1>=read_len:
                break

        for k in ORGANISM2ID2.keys():
            store[k]=''
        all_data=pd.DataFrame(store)
        print(all_data.columns)
    return all_data


def split_dataset_by_structure(dataset_path,valid_ratio=0.1, min_seq_id=0.3, cover=0.8):
    if not os.path.isfile(dataset_path):
        raise FileExistsError(f'{dataset_path} is not an existent file.')
    if dataset_path.split('.')[-1] not in ['json','jsonl']:
        raise ValueError('Dataset format only supports json and jsonl files.')
    dir=os.path.dirname(dataset_path)
    dataset=IterableJSONData(dataset_path)
    ...
    for item in dataset:
        if item.get('organism') is None:
            return None, None
        else:
            break
    for item in dataset:
        seq_aa = ''.join([c.split(STOP_SYMBOL)[-1] for c in item['codons'].split(' ')]).upper()
        ds_by_org[ID2ORGANISM2[item['organism']]].append(item)
        ds_by_org[ID2ORGANISM2[item['organism']] + '_aa'].append(seq_aa)

    """
    max_l=max([len(k) for k in ds_by_org.keys()])
    for k in ds_by_org.keys():
        if len(ds_by_org[k])<max_l:
            ds_by_org[k].extend([None for _ in range(max_l-len(ds_by_org[k]))])
    """

    total_trds, total_vads = [], []
    set_verbosity(verbose=True)
    for k in ORGANISM2ID2.keys():
        df = pd.DataFrame(data={k: ds_by_org[k], k + '_aa': ds_by_org[k + '_aa'],"index":list(str(i) for i in range(len(ds_by_org[k])))})
        print(df[k+'_aa'].dtype)
        print(df[k+'_aa'].head())
        # df[k+'_aa']=df[k+'_aa'].astype(str)
        # print(df[k+'_aa'].dtype)
        clustered_df = cluster(df, sequence_col=k + '_aa', id_col='index', min_seq_id=min_seq_id, coverage=cover)
        train_ds, valid_ds = split(clustered_df, test_size=valid_ratio)
        train_ds = train_ds[k].to_list()
        # train_ds = train_ds[k].values.to_list()
        print(train_ds[0])
        valid_ds = valid_ds[k].to_list()
        # valid_ds = valid_ds[k].values.to_list()
        total_trds.extend(train_ds)
        total_vads.extend(valid_ds)

    trfn=os.path.join(dir,f'training_data_{(1-valid_ratio):.1f}_{min_seq_id:.1f}_{cover:.1f}.jsonl')
    vafn=os.path.join(dir,f'valid_data_{valid_ratio:.1f}_{min_seq_id:.1f}_{cover:.1f}.jsonl')
    tr_count,va_count=0,0
    with open(trfn,'w+') as f:
        for item in total_trds:
            # print(item)
            assert(item['idx'] is not None)
            assert(item['codons'] is not None)
            assert(item['organism'] is not None)
            f.write(json.dumps(item)+'\n')
            tr_count+=1
    print(f'{tr_count} items successfully written to {trfn}.')

    with open(vafn,'w+') as f:
        for item in total_vads:
            assert(item['idx'] is not None)
            assert(item['codons'] is not None)
            assert(item['organism'] is not None)
            f.write(json.dumps(item)+'\n')
            va_count+=1
    print(f'{va_count} items successfully written to {vafn}.')

    return total_trds, total_vads
