        ...
        if train and focus is not None:
            ...
            for i in range(NUM_ORGANISMS):
                ...
                if len(self.ref_trgcs[i]) > 0:
                    ...
                    self.ref_trgcs[i].append(
                        (self.ref_trgcs[i][-1] * len(self.ref_trgcs[i]) + gc_record[i] / (number + 1e-3)) / (
                                    len(self.ref_trgcs[i]) + 1))
                    self.ref_trcais[i].append(
                        (self.ref_trcais[i][-1] * len(self.ref_trcais[i]) + cai_record[i] / (number + 1e-3)) / (
...
class MyDataLoader(pl.LightningDataModule):
    def __init__(self, train_dataset=None, valid_dataset=None, test_dataset=None, collate_fn=None, batch_size=6,
        ...
        self.test_dataset = list(test_dataset) if test_dataset is not None else None
...
class MyBigBirdModel(BigBirdForMaskedLM):
    ...
    def forward(
        ...
        if labels is not None and token_type_ids is not None:
            ...
            masked_lm_loss = loss_fct(prediction_scores, labels) + self.type_lf * loss_fct(
                prediction_organism_scores.view(-1, prediction_organism_scores.shape[-1]),
                tlabels.view(-1))
...
def main(args):
    ...
    if args.ckpt_path != "":
        if os.path.isfile(args.ckpt_path):
            harnessed_model = harnessed_model.load_from_checkpoint(args.ckpt_path, model=model, tokenizer=tokenizer,
    ...
    counts,ds_by_org=split_dataset_by_organism(train_data)
    assert(sum(counts)!=-1 and (len(ds_by_org[i])>0 for i in range(NUM_ORGANISMS)))


    # Setup datamodule
    # plt.ion()
    for k,i in ORGANISM2ID2.items():
        # 验证集基于同一物种，中间逐物种训练结果模型在其他物种上的表现可以通过inference.py测试
        dataloader = MyDataLoader(
            train_dataset=ds_by_org[i],
        ...
        max_epochs_i=max(int(args.max_epochs*counts[i]),1)
        logger = CSVLogger(os.path.join(args.checkpoint_dir,k), name=None)
        # 每个物种结束后保存模型（中间结果）
        save_checkpoint = EpochCheckpoint(os.path.join(args.checkpoint_dir,k), max(1,max_epochs_i-1))
        trainer = pl.Trainer(
            logger=logger,
            strategy="dp",
            accelerator="gpu",
    ...
    # 为避免物种顺序随机问题，单独多保存一份最终结果
    torch.save(harnessed_model.state_dict(),os.path.join(args.checkpoint_dir,'final_model_weights.pth'))
    ...
    if args.conclude:
        ...
        # 每个物种结束后保存模型（中间结果）
        save_checkpoint = EpochCheckpoint(args.checkpoint_dir, args.conclude_epochs-1)
        trainer = pl.Trainer(
            logger=logger,
            strategy="dp",
            accelerator="gpu",
            devices=1 if args.debug else args.num_gpus,
            precision="bf16",
            max_epochs=args.conclude_epochs,
            deterministic=False,
            enable_checkpointing=False,
            callbacks=[save_checkpoint],
            accumulate_grad_batches=args.accumulate_grad_batches,
...
def split_dataset_by_organism(dataset):
    assert(isinstance(dataset,IterableJSONData))
    counts=np.array([0 for i in range(NUM_ORGANISMS)],dtype=float)
    ds_by_org=[[] for i in range(NUM_ORGANISMS)]
    for item in dataset:
        if item.get('organism') is None:
            return -1
    for item in dataset:
        counts[item.get('organism')]+=1
        ds_by_org[item.get('organism')].append(item)
    print(f'counts:{counts}')
    counts/=sum(counts)

    return counts,ds_by_org
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--conclude", type=bool, default=True, help="Whether to train on the whole dataset finally"
    )
    parser.add_argument(
        "--conclude_epochs", type=int, default=5, help="Number of comprehensive training epochs"
    )
    # --debug改为store_false，在多N卡机上可以提升训练效率
